...
2. 展示实验迭代路线图，说明项目从二进制读取逐步发展到交互分析。
3. 点击播放按钮，展示宇宙密度随时间演化。
4. 切换到 t=0000、t=0030、t=0060、t=0099，对比空间结构变化。
5. 展示 histogram，说明密度分布逐渐扩散，高密度尾部增强。
6. 将 density range 设置为 99% 到 100%，展示 top 1% 高密度体素。
7. 切换 X/Y/Z 投影方向，说明高密度体素集中在节点和丝状交汇处。
8. 展示 time-density heatmap，说明全时间步密度分布变化。
9. 展示统计曲线，说明 std、P99、P99/mean 和 P99-P01 的变化。
10. 最后展示结论卡片，说明统计特征与空间结构的双向关联。
